...
If the receipt shows a tip, use it. If it doesn't, ask what tip they want before
building anything — don't assume 20%. Compute the dollar amount on the
**pre-tax subtotal** unless they say otherwise.

If the receipt has an auto-gratuity or service charge, put it in `tip` and say
so, so nobody double-tips.
...
Show the parsed table — items, quantities, prices, tax, tip, total — and ask the
user to confirm before you give them the URL. Receipt OCR misreads `0` as `O`
and drops decimal points, and this is the only place to catch it.

Only after they confirm, give them:

1. The **host link** — copy the `host url:` line printed by `encode.js`
   **exactly as printed.** Never retype the fragment or append `.h` by hand;
   the fragment is a ~250+ character opaque string and any transcription
   introduces a single wrong character that silently breaks the link for
   every guest. `encode.js` prints the finished `guest url:` and `host url:`
   lines for exactly this reason — always relay one of those two lines
   verbatim, character for character, never a reconstructed version.
2. A one-line note that guests get the plain URL automatically when they scan.

Don't paste both links unless asked; the host link is all they need.
