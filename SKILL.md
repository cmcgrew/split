...
payment tracker — this is theirs to open; items are read-only there, since
they already paid) and **GUEST LINK** (what a scanned QR resolves to; only
worth sending separately if they're not using the QR).
