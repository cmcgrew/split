...
```bash
node encode.js bill.json --out=/mnt/user-data/outputs/link.html
```

This writes both finished URLs straight from memory to disk — the same
strings `encode.js` validated, never retyped — as a small styled card with
two tappable buttons: **HOST LINK** (opens the QR code and payment
tracker — this is theirs to open) and **GUEST LINK** (what a scanned QR
resolves to; only worth sending separately if they're not using the QR).
Use `present_files` to hand that file to the user; it renders inline as a
clickable artifact, not a plain download. `--out=` also accepts a `.md`
path if a markdown link pair is ever preferred instead. **Do not
